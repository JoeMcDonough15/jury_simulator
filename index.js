...
// tunesOrProficienciesSelect = prompt('Would you like to practice Tunes (t) or Proficiencies (p)?);
...
// first challenge 
const selectionNum = Math.floor(Math.random() * material.proficiencies.length);
const selection = material.proficiencies[selectionNum];
...
// Generate Selection - generate random number and index material.proficiencies at that number
// loop to play game 
function gameLoop() { // an array from materials containing either proficiencies or tunes
    const selectionNum = Math.floor(Math.random() * material.proficiencies.length);
    const selection = material.proficiencies[selectionNum];
    const fixedSelection = selection.replace('INTERPOLATE_KEY_HERE', `${generateKey()}`)
    headerLevel.innerText = `Level ${material.level}`;
...
document.addEventListener('keydown', e => {
    switch(e.key) {
        case '1' : 
            ...
            gameLoop(material.proficiencies);
            break;
        case '2' :
